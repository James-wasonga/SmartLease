    ...
    <div>
      ...
      >
        <div className="w-full relative">
          <Badge
            className={cn(
              "absolute right-4 top-4 p-1 bg-green-500 hover:bg-green-500 z-10",
              { "bg-gray-400 hover:bg-gray-400": !is_occupied }
            )}
          >
            {is_occupied ? "Occupied" : "Unoccupied"}
          </Badge>

          <PropertyImage
            src={property_image_url}
            alt={`${name} property image`}
            className="h-44 w-full object-cover object-center rounded-t-lg"
          />
        </div>
        ...
        <CardContent className="text-sm">
          <div className="border-t border-b py-2 flex justify-between items-center">
            <span className="text-gray-500">
              Token ID
              <span className="text-slate-700 font-medium"> #{token_id}</span>
            </span>
          ...
          <div className="flex items-end justify-between mt-3">
            ...
            <span className="text-gray-400">
              Flexible:{" "}
              <span
                className={cn("text-green-500 font-medium", {
                  "text-red-500": !flexible_payment,
                })}
              >
                {flexible_payment ? "Yes" : "No"}
              </span>
            </span>
          </div>
        </CardContent>
        ...
        <CardFooter>
          <Link
            href={`${Routes.LANDLORD_PROPERTIES}/${proprtyId}?tokenId=${token_id}`}
          >
